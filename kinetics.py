...
def make_dataset(root_path, annotation_path, class_labels, subset, n_samples_for_each_video, sample_duration):
    ...
    pre_saved_dataset = os.path.join(root_path, 'labeldata_80_weaklySup.npy')
...
class Kinetics(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = (torch.ones(len(frame_indices)) * target).long() # T

        if self.crops==1:
            clips = torch.stack([clip], 0)
            targets = torch.stack([target], 0)# N T
        else:
            if step == 0:
                clips = [clip[:,:self.frames,...] for i in range(self.crops)]
                clips = torch.stack(clips, 0)
                #targets = [target[:self.frames,:] for i in range(self.crops)]
                #targets = torch.stack(targets, 0).transpose(-1,-2)
                targets = [target[:self.frames] for i in range(self.crops)]
                targets = torch.stack(targets, 0) # N T
            else:
                clips = [clip[:,i:i+self.frames,...] for i in range(0, step*self.crops, step)]
                clips = torch.stack(clips, 0) # N C T H W
                #targets = [target[i:i+self.frames,:] for i in range(0, step*self.crops, step)]
                #targets = torch.stack(targets, 0).transpose(-1,-2) # N T C_
                targets = [target[i:i+self.frames] for i in range(0, step*self.crops, step)]
                targets = torch.stack(targets, 0)# N T

        return clips, targets, self.data[index]['video_id'], self.class_names[self.data[index]['label']]
